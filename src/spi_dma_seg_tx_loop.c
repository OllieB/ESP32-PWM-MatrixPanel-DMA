...
#include <soc/spi_periph.h>    // spi_periph_signal - to rebind GCLK pin to the SPI peripheral
#include <soc/gpio_sig_map.h>  // SIG_GPIO_OUT_IDX - to park GCLK low via the GPIO matrix
...
static inline void poll_spi_seg_status(void)
{
  ...
  // segment completes. Only the latter means the GCLK/ADDR engine has halted - and it always
  // halts at a segment boundary (end of row 19's dead time), never mid-row. Gating on trans_done
  // (as previously) released spi_transfer_loop_stop()'s busy-wait at an ordinary segment boundary
...
esp_err_t spi_transfer_loop_start()
{
  ...
  // Hand the GCLK pin back to the SPI peripheral (it is parked low via the GPIO matrix while the
  // loop is stopped, so the Vsync command sees GCLK held low as the MBI5153 requires).
  esp_rom_gpio_connect_out_signal(MBI_GCLK, spi_periph_signal[SPI2_HOST].spid7_out, false, false);
...
esp_err_t spi_transfer_loop_stop(void) {
  ...
  // Reset BEFORE requesting the stop. spi_seg_transfer_complete is otherwise left over
  // as 'true' from the free-running loop's last ordinary (non-final) segment completion, which
  // would make the wait below fall through immediately instead of waiting for the genuine halt.
  spi_seg_transfer_complete = false;
  ...
  // Vsync command. Park the GCLK pin low via the GPIO matrix until spi_transfer_loop_start()
  // hands it back to the SPI peripheral.
  gpio_set_level((gpio_num_t)MBI_GCLK, 0);
  esp_rom_gpio_connect_out_signal(MBI_GCLK, SIG_GPIO_OUT_IDX, false, false);
  gpio_set_direction((gpio_num_t)MBI_GCLK, GPIO_MODE_OUTPUT);
  esp_rom_delay_us(1);  // comfortably satisfies the >300ns GCLK-low-before-Vsync requirement
